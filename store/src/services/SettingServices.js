...
const getStoreSecretKeys = async () => {
  try {
    // Send the shared secret (server-side only) so the backend's optional
    // STORE_API_SECRET guard lets this through. Harmless if unset.
    const secretHeaders = process.env.STORE_API_SECRET
      ? { "x-store-api-key": process.env.STORE_API_SECRET }
      : {};
