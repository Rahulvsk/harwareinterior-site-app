      ...
      {error ? (
        ...
        <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-6">
          {categories[0]?.children?.map((category, i) => (
            ...
            // <li className="group m-2" key={i + 1}>
...
>
  <div
    className="flex w-full h-40 border border-gray-100 dark:border-gray-950 shadow-sm bg-white dark:bg-zinc-900 p-4 cursor-pointer transition duration-200 ease-linear transform group-hover:shadow-lg bg-cover bg-center rounded-lg"
    style={{
      backgroundImage: `url(${category?.icon || "https://res.cloudinary.com/ahossain/image/upload/v1655097002/placeholder_kvepfp.png"})`,
    }}
  >
    <div className="flex items-center bg-black/40 w-full h-full rounded-lg p-2">
      <CategoryNavigateButton
        category={{
          ...category,
          name: category.name,
